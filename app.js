...
const SCRIPT_URL = "https://script.google.com/macros/s/AKfycbx7LJkaVcE_M7ym82prhTkS7xQMTUnQUvf4XKtlmbgE8qxVPqb1noae_rxYU1_sBXZZ/exec";
...
// Diperbarui: Memberikan default properti date agar tidak merusak fungsi filter keuangan saat pertama dibuka
let orders = [
...
function loadDataFromCloud() {
    ...
    fetch(`${SCRIPT_URL}?action=read`)
        ...
        .then(cloudData => {
            ...
            // 2. SINKRONISASI DAFTAR TRANSAKSI NOTA + AMBIL TANGGAL DARI CLOUD
            if (cloudData.transactions && cloudData.transactions.length > 0) {
                ...
                // =========================================================================
                // 💡 LOGIKA TAMBAHAN: EKSTRAKSI DAFTAR PELANGGAN OTOMATIS DARI NOTA SPREADSHEET
                // =========================================================================
                ...
                // Jika di Google Sheets sudah ada riwayat transaksi, gunakan data tersebut sebagai database pelanggan
            ...
            renderServicesGrid();
            populateDropdowns(); // 💡 Memastikan dropdown pelanggan langsung digambar ulang dengan data dari cloud
            renderOrders();
            calculateFinance();
...
function populateDropdowns() {
    ...
    if (custDropdown) {
        if (customers && customers.length > 0) {
            // VALUE KITA KUNCI HANYA C.NAME AGAR PENCARIAN DI CHECKOUT AKURAT
    ...
    if (payDropdown && paymentMethods) {
        payDropdown.innerHTML = paymentMethods.map(p => `
...
// PROSES EKSEKUSI TRANSAKSI CHECKOUT NOTA KASIR (DIAMANKAN)
function processCheckout() {
    ...
    if (isNewCustomerMode) {
        ...
        // Konversi otomatis jika input berawalan '08' menjadi '628'
        customerPhone = inputPhone.startsWith('0') ? '62' + inputPhone.slice(1) : inputPhone;
        customerPhone = customerPhone.replace(/[^0-9]/g, ''); // Pastikan hanya angka bersih

        customers.push({ id: `C${customers.length + 1}`, name: customerName, phone: customerPhone });
        populateDropdowns();
    } else {
        // AMBIL NAMA BERSIH DARI DROPDOWN VALUE
        customerName = document.getElementById('cart-customer').value;
        // CARI SECARA AKURAT BERDASARKAN NAMA KONSUMEN
    ...
    // Masukkan ke memori lokal & render ulang halaman kasir agar instan muncul
...
function sendWhatsAppReceipt() {
    ...
    const total = `Rp ${Number(order.total).toLocaleString('id-ID')}`;
    ...
    const messageText =
`Halo, Terima kasih telah mencuci di *Forresa Laundry*.
...
🌿 Pantau status proses pengerjaan laundry pakaian Anda secara realtime melalui link resmi berikut:
🔗 ${trackingUrl}`;

    window.open(
        `https://api.whatsapp.com/send?phone=${customerPhone}&text=${encodeURIComponent(messageText)}`,
        '_blank'
    );
}
    ...
    ordersList.innerHTML = orders.map(o => {
        ...
        return `
            <div class="bg-white p-5 rounded-2xl border border-slate-100 shadow-xs space-y-3 relative">
                ...
                <button onclick="deleteOrderFromPOS('${o.id}')" class="absolute top-4 right-4 text-xs text-rose-500 bg-rose-50 w-7 h-7 rounded-full hover:bg-rose-100 flex items-center justify-center transition-all z-20" title="Hapus Nota Ini">
...
// FUNGSI INTI: PROSES HAPUS TRANSAKSI NOTA DUA ARAH
function deleteOrderFromPOS(orderId) {
    if (confirm(`Apakah Anda yakin ingin menghapus Nota ${orderId} secara permanen dari daftar kasir dan database cloud Spreadsheet?`)) {
        // 1. Hapus secara lokal dari memori web kasir saat ini
        orders = orders.filter(o => o.id !== orderId);
        
        // Render ulang daftar nota dan hitung ulang rekap keuangan agar langsung sinkron
        renderOrders();
        calculateFinance();

        // 2. Kirim perintah hapus ke cloud Google Sheets
        if (SCRIPT_URL !== "" && !SCRIPT_URL.includes("SCRIPT_URL")) {
            const deletePayload = {
                action: "deleteOrder",
                id: orderId
            ...
            .then(() => console.log(`Nota ${orderId} sukses dihapus dari database cloud.`))
            .catch(err => console.error("Gagal hapus nota di cloud:", err));
        }

        triggerNotification(`Nota transaksi ${orderId} berhasil dihapus.`);
    }
}
...
function updateOrderStatus(orderId, newStatus) {
    ...
    if (orderIndex !== -1) {
        ...
        if (SCRIPT_URL !== "" && !SCRIPT_URL.includes("SCRIPT_URL")) {
            const updatePayload = { action: "updateStatus", id: orderId, status: newStatus };
            fetch(SCRIPT_URL, {
                method: 'POST',
                mode: 'no-cors',
                headers: {'Content-Type': 'application/json'},
...
// FUNGSI INTI: MENGHITUNG DAN MENYARING OMZET KEUANGAN LAUNDRY
    ...
    if (mode === 'today') {
        ...
        if (pickerMonth) {
            filteredOrders = orders.filter(o => {
                ...
                const orderMonthStr = `${orderYear}-${orderMonth}`; 
...
// AUTO REFRESH REAL-TIME KHUSUS PELANGGAN VIA LINK PARAMETER
