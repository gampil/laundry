...
function exportToExcel() {
  if (!CACHE_DATA.transaksi || CACHE_DATA.transaksi.length === 0) { 
    alert("Data transaksi masih kosong, tidak ada yang bisa diekspor."); 
    return; 
  }

  try {
    // 1. Pemetaan data agar rapi saat dibaca di Microsoft Excel / WPS Office
    const cleanObj = CACHE_DATA.transaksi.map((t, i) => ({
      ...
      "Berat / Qty": Number(t.berat) || 0,
      "Total Omset (Rp)": Number(t.total) || 0,
    ...
    // 2. Membuat worksheet baru dari objek data di atas
    const ws = XLSX.utils.json_to_sheet(cleanObj);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Laporan Kasir");

    // 3. Menulis file Excel ke dalam array biner buffer (Metode paling kompatibel di HP)
    const wbout = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
    
    // 4. Membungkus array biner menjadi file Blob yang sah
    const blob = new Blob([wbout], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" });
    const namaFile = `Laporan_Laundry_${new Date().toISOString().split('T')[0]}.xlsx`;

    // 5. Membuat link jangkar tak terlihat untuk memaksa HP mengunduh berkas
    ...
    // Eksekusi unduhan otomatis
    a.click();
    
    // Bersihkan kembali sisa memori browser
    setTimeout(() => {
    ...
    console.error("Detail Error Excel:", error);
